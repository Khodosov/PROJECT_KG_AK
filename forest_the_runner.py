import pygame
from levels import l1, l2, l3, l4, l5, l6, l7, l8, l9, Platform #li (1<=i<=9) - это список платформ - уровень, из файла levels.py. Platform - класс платформ
...
#След. две переменные запихиваем в класс Player (соотв-но они теперь обладают всеми свойствами класса)
pl2 = Player(x2, y2, player2)
pl1 = Player(x1, y1, player1)

left1 = left2 = right1 = right2 = False # переменные, отвечающие, движется ли игрок вправо/влево
...
sprite_group = pygame.sprite.Group() #переменная (группа), в которой содержатся все элементы класса Sprite (и игроки, и платформы)
...
lev = l9 #l9 выбрано для примера. Саша, напиши код так, чтобы в зависимости от нажатой кнопки (1, 2, ... 9) выбирался нужный список (уровень - l1, l2,... l9)
# Координаты платформ:
x = 0 
y = 0
...
for i, j in enumerate(lev): #движение по столбцам
    for k, l in enumerate(j): #движение по строке
        if l == '-':
            pl = Platform(x, y) #переменная, отвечающая за расположение в координатах каждой новой платформы
            sprite_group.add(pl) #добавление в группу новой платформы
            platforms.append(pl) #добавление в список платформ новой платформы
    ...
    while run1 or run2:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run1 = False
                run2 = False
            if event.type == pygame.KEYDOWN: #проверка, происходит ли НАжатие кнопки (в данный момент)
                ...
                if event.key == pygame.K_w:
                    up2 = True
            elif event.type == pygame.KEYUP: #проверка, произошло ли ОТжатие кнопки (в данный момент)
                if event.key == pygame.K_LEFT:
                    left1 = False
                if event.key == pygame.K_RIGHT:
                    right1 = False
                ...
                if event.key == pygame.K_w:
                    up2 = False
        pl1.update(left1, right1, up1, platforms)#метод обноления в классе Player
        pl2.update(left2, right2, up2, platforms)#^^^^^
        sprite_group.draw(win)#отрисовка ВСЕХ элементов группы (и платформ, и игроков)
        pygame.display.flip()
        timer.tick(60)
